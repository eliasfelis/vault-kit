...
| `tier` | yes | Grouping label for display (`1`, `2`, `3`, or `custom`). |
...
| `paths.state` | `.feed-state` | Directory for per-source run-state (last-seen epoch, outcome). |
...
| **news-tier** | One paragraph in the daily digest (`paths.digest`) | Worth knowing, not worth a task. |
...
All paths come from your config (defaults shown):
...
| `feed-digest/` | Daily digest files: `<date>.md`. One paragraph per news-tier item. |
| `.feed-state/` | Per-source run-state (last-seen epoch + outcome). Used to avoid re-surfacing seen items. |
